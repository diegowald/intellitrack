...
namespace SharpMap.Layers
{
	...
	public class LabelLayer : Layer, IDisposable
	{
		...
		protected SharpMap.Rendering.LabelCollisionDetection.LabelFilterMethod _LabelFilter;
		...
		protected GetLabelMethod _getLabelMethod;
		...
		protected virtual SharpMap.Rendering.Label CreateLabel(SharpMap.Geometries.Geometry feature,string text, float rotation, SharpMap.Styles.LabelStyle style, Map map, System.Drawing.Graphics g)
		{
			System.Drawing.SizeF size = g.MeasureString(text, style.Font);
      SharpMap.Geometries.BoundingBox bbox = feature.GetBoundingBox();
      if (bbox == null)
        return null;
      System.Drawing.PointF position = map.WorldToImage(bbox.GetCentroid());
		...
		protected void CalculateLabelOnLinestring(SharpMap.Geometries.LineString line, ref SharpMap.Rendering.Label label, Map map)
		{
			double dx, dy;
			double tmpx, tmpy;
			double angle = 0.0;
